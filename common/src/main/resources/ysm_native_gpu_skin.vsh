...
void main() {
    ...
    vec4 eyePos = bone.transform * vec4(a_position, 1.0);
    vec3 normal = normalize((bone.normal * vec4(a_normal.xyz, 0.0)).xyz);
    gl_Position = u_proj * eyePos;
    v_uv = a_uv;
    v_color = minecraft_mix_light(u_light0, u_light1, normal, u_color);
    v_vertexDistance = fogDistance(eyePos.xyz, u_fogShape);
    v_cullable = a_cullable;
    v_packedLight = bone.packedLight;
}
